...
public class ImController {
    ...
    @ApiOperation(value= "医生向患者推送信息，包括文字、图片、语音，支持多@推送",notes = "医生向患者推送信息，包括文字、图片、语音，支持多@推送")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "roomId",value= "房间id",paramType = "query",required = true, dataType = "String"),
            @ApiImplicitParam(name = "type",value= "消息类型1->文字 2->图片 3->语音 ",paramType = "query",required = true, dataType = "String"),
            @ApiImplicitParam(name = "token", value = "用户",paramType="header", required = true, dataType = "String"),
            @ApiImplicitParam(name = "sendCrowd",value= "发送对象 1医生 2患者 ",paramType = "query",required = false,defaultValue = "1",dataType = "String"),
            @ApiImplicitParam(name = "isAit",value= "是否@推送  1是2否 ",paramType = "query",required = false,defaultValue = "2" ,dataType = "String"),
            @ApiImplicitParam(name = "doctors",value= "@推送状态下 医生id '1_2_3_4'",paramType = "query",required = false,dataType = "String")
    })
    ...
    public AppResult doctorPushMessageToPatients(@RequestParam String roomId,
                                                 ...
                                                 @RequestParam(value = "sendCrowd",required = false,defaultValue ="1") String sendCrowd,
                                                 @RequestParam(value = "isAit",required = false,defaultValue ="2") String isAit,
                                                 @RequestParam(value = "doctors",required = false) String doctors){

        return this.imService.doctorPushMessageToPatient(roomId,type,token,sendCrowd,isAit,doctors);
    }


    @ApiOperation(value = "定时任务每天晚间9点发送预约提醒", notes = "定时任务每天晚间9点发送预约提醒")
    @GetMapping("/reservationReminding")
    public AppResult reservationReminding(){

        return this.imService.reservationReminding();
