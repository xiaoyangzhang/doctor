...
public class DepartmentServiceImpl implements DepartmentService {
    ...
    @Value("${EXHIBIT_URL}")
    private String EXHIBIT_URL;
    @Value("${TYPICALILL_URL}")
    private String TYPICALILL_URL;
...
//        List<ItemResultDTO> itemDTOs = departmentDTO.getItemDTOs();
//
//        if (!CollectionUtils.isEmpty(itemDTOs)){
//            ArrayList<Item> items = new ArrayList<>();
//            for (ItemResultDTO dto:itemDTOs){
//                Item item = new Item();
//                item.setId(dto.getId());
//                item.setState(dto.getState());
//                item.setUpdateTime(new Date());
//                if (dto.getState()==2){
//                    item.setOnshelfDay(new Date());
//                }else if (dto.getState()==4){
//                    item.setOffshelfDay(new Date());
//                }
//                items.add(item);
//            }
...
//            HttpEntity<List> httpEntity = new HttpEntity<>(items, headers);
//            restTemplate.patchForObject(pathConfiguration.getSystemUrl()+"/item/update/state",httpEntity, com.yhyt.health.spring.AppResult.class);
//
//        }
        return 0;
    }
